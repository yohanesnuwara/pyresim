...
def boundary_floweq1d(bound_type, dx, dy, dz, kx, mu, B, p_b=1000, p_grad=0.1, q_b=-200, no_block=1):
    ...
    p_b, p_grad, q_b = specify only one of these variables, depends on the boundary conditions
...
def boundary_floweq2d(bound_type, bound_loc, dx, dy, dz, kx, mu, B, p_b=1000, p_grad=0.1, q_b=-200, no_block=(2,1), no_blocks_shared=4):
    """
    ...
    * if B.C. is 'no_flow',
    ...
    if bound_type == 'constant_pressuregrad':
        Ax = dy * dz
        if bound_loc == 'south' or bound_loc == 'west':
            # characterize as Left boundary
            qsc = '{}'.format(np.round(-(.001127 * (kx * Ax) / (mu * B)) * (p_grad - 0), 5))  # Eq 4.45, zero (p_grad - 0) because Z1=Z2
        if bound_loc == 'north' or bound_loc == 'east':
            # characterize as Right boundary
            qsc = '{}'.format(np.round((.001127 * (kx * Ax) / (mu * B)) * (p_grad - 0), 5))  # Eq 4.45, zero (p_grad - 0) because Z1=Z2
    if bound_type == 'constant_rate':
        qsc = (1 / no_blocks_shared) * q_b
    if bound_type == 'no_flow':
        qsc = 0
    return(qsc)

def boundary_floweq3d(bound_type, bound_loc, dx, dy, dz, kx, mu, B, rho, p_b=1000, p_grad=0.1, q_b=-200, no_block=(2,1,3), no_blocks_shared=4):
    """
    Boundary Conditions for 2D Rectangular Reservoir (floweq simulation)
    ...
    * if B.C. is 'no_flow', don't specify any value
    ...
    > boundary_floweq2d('constant_pressure', 'bottom', dx=dx, dy=dx, dz=dz, kx=kx, mu=mu, B=B, rho=rho, p_grad=0.1, no_block=(5,4), no_blocks_shared=7)
    ...
    * if B.C. is 'no_flow',
    ...
    if bound_type == 'constant_pressuregrad':
        ...
        if bound_loc == 'bottom':
            ...
            gamma = .21584E-3 * rho * 32.174
            Z_gamma = Z * gamma
            qsc = '{}'.format(np.round(-(.001127 * (kx * Ax) / (mu * B)) * (p_grad - Z_gamma), 5))
        if bound_loc == 'upper':
            Z = -0.5 * dz
            gamma = .21584E-3 * rho * 32.174
            Z_gamma = Z * gamma
