...
def constant_pressure_bc3d(bound_loc, no_block, p_b, dy, dz, kx, dx, mu, B, rho):
  """
  Flow expression for 3D constant pressure boundary block
...
def constant_pressuregrad_bc3d(bound_loc, p_grad, dy, dz, kx, mu, B, rho):
  """
  Flow expression for 3D constant pressure gradient boundary block

  (Actually, it's not discussed in Abou-Kassem book.) 
  
  Here, I still assume that for flow to either BOTTOM or UPPER boundary block, 
  term (γ * ∂Z/∂z) is not 0. (∂Z/∂z) equals to 0.5*dz (half of grid height)
  Except both BOTTOM or UPPER, term (γ * ∂Z/∂z) equals to 0.
