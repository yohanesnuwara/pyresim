...
# boundary conditions
pg_bW = 0.1 # pressure gradient at left (West) boundary
q_bE = 50. # flow rate at right (East) boundary
...
for i in range(len(block)):
  ...
  T_plus = T_min 
  if i == 0:
    # constant pressure gradient at left boundary
    qsc_b = constant_pressuregrad_bc1d('west', p_grad, dy[i], dz[i], kx[i], mu[i], B[i])
    print('Boundary Block {}: {} (p{} - p{}) + {} + {}'.format(block[i], T_plus, block[i+1], block[i], qsc_b, qsc[i]))
  elif i == len(block)-1:
    # specified flow at right boundary
